...
class Ttmegamenu extends Module implements WidgetInterface
{
    ...
    public function __construct()
    {
        ...
        $this->bootstrap = true;
        parent::__construct();
        $this->displayName = $this->l('TT - Megamenu');
        $this->description = $this->l('Show Megamenus on your site');
    ...
    {
        if (parent::install() && $this->registerHook('header') && $this->registerHook('displayMegamenu') && $this->registerHook('displayMegamenuMobile') && $this->registerHook('actionShopDataDuplication') && $this->registerHook('actionObjectLanguageAddAfter') && $this->registerHook('displayMegamenu')&& $this->registerHook('displayNav2')) {
...
//        $languages = Language::getLanguages(false);
    ...
    public function hookHeader()
    {
        ...
        $this
            ->context
            ->controller
            ->addCSS($this->_path . 'views/css/ttmegamenu.css');
    }
...
//            $languages = Language::getLanguages();
    ...
    public function getRespCategories($id_category = 1, $id_lang = false, $id_shop = false, $level_root = 1, $sub_title = '', $type_icon = 1, $icon = '', $sub_align = '', $spec_class = '')
    {
        ...
        if (!empty($is_intersected)) {
            $this->respMenu .= '<li class="cat-drop-menu-sub ' . $class . '">';
            $this->respMenu .= '<a class="ttinnermenu" href="' . $cat_link . '">';
            if ($type_icon == 1 && $icon != '') {
                $this->respMenu .= '<i class="material-icons">' . $icon . '</i>';
            } elseif ($type_icon == 0 && $icon != '') {
                $this->respMenu .= '<img src="' . $icon_path . $icon . '" alt="image.jpg">';
            }
            $this->respMenu .= $category->name;
            if ($sub_title != '') {
