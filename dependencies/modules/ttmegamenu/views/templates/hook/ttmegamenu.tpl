...
{$id_lang = Context::getContext()->language->id}
	...
	<ul class="menu-content">
		{foreach from=$menus item=menu name=menus}
			...
			{else}
			{if $menu.title != ''}
				<li class="{$menu.class|escape:'html':'UTF-8'}{if count($menu.sub_menu) > 0} parent{/if}">
					...
					{if count($menu.sub_menu) > 0}<span class="icon-drop-mobile"></span>{/if}
											...
											<div class="tt-menu-col col-xs-12 {$menu_col.width|escape:'html':'UTF-8'} {$menu_col.class|escape:'html':'UTF-8'}">
												{if count($menu_col.list_menu_item) > 0}
													...
													{foreach from=$menu_col.list_menu_item item= sub_menu_item name=sub_menu_item}
														...
														{if $sub_menu_item.title}<a href="{$sub_menu_item.link|escape:'html':'UTF-8'}">{$sub_menu_item.title|escape:'html':'UTF-8'}</a>{/if}
