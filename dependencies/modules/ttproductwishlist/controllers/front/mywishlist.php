...
class TtProductWishListMyWishListModuleFrontController extends ModuleFrontController
{
    public $php_self;
    ...
    */
    public function assign()
    {
        ...
        if ($this->context->customer->isLogged()) {
            ...
            if (Tools::isSubmit('submitWishlist')) {
                ...
                if (!count($errors)) {
                    ...
                    if (TtWishList::isExistsByNameForUser($name)) {
                    ...
                    if (!count($errors)) {
                        ...
                        Mail::Send(
                            ...
                            array(
                                ...
                                '{message}' => $this->context->link->getModuleLink('ttproductwishlist', 'view', array('token' => $wishlist->token))
                                ),
                        ...
                        Tools::redirect($this->context->link->getModuleLink('ttproductwishlist', 'mywishlist'));
                    }
                }
            } elseif ($add) {
                TtWishList::addCardToWishlist($this->context->customer->id, Tools::getValue('id_wishlist'), $this->context->language->id);
            } elseif ($delete && empty($id_wishlist) === false) {
                $wishlist = new TtWishList((int)$id_wishlist);
            ...
            $this->context->smarty->assign('wishlists', TtWishList::getByIdCustomer($this->context->customer->id));
            $this->context->smarty->assign('nbProducts', TtWishList::getInfosByIdCustomer($this->context->customer->id));
        } else {
            Tools::redirect('index.php?controller=authentication&back='.urlencode($this->context->link->getModuleLink('ttproductwishlist', 'mywishlist')));
        }
        ...
        $this->setTemplate('module:ttproductwishlist/views/templates/front/mywishlist.tpl');
    }
    ...
    public function ajaxProcessDeleteList()
    {
        ...
        if ($default_change) {
            $array = TtWishList::getDefault($id_customer);
    ...
    public function ajaxProcessProductChangeWishlist()
    {
        ...
        $new_wishlist = new TtWishList((int)$id_new_wishlist);
        $old_wishlist = new TtWishList((int)$id_old_wishlist);
        ...
        $res = true;
        $check = (int)Db::getInstance()->getValue('SELECT quantity FROM '._DB_PREFIX_.'ttwishlist_product
