...
class MigrateData
{
    ...
    {
        $this->loadedData = [];
        $texts = Db::getInstance()->executeS('SELECT i.`id_shop`, il.`id_lang`, il.`text` FROM `' . _DB_PREFIX_ . 'ttcmsfooterinfo` i
            INNER JOIN `' . _DB_PREFIX_ . 'ttcmsfooterinfo_lang` il ON il.`id_ttcmsfooterinfo` = i.`id_ttcmsfooterinfo`');
    ...
    public function insertData()
    {
        ...
        $ttcmsFooter = array_intersect_key($this->loadedData, $shopsIds);

        $info = new ttcmsfooterinfo();
        $info->text = reset($ttcmsFooter);
        $return &= $info->add();

        if (count($ttcmsFooter) > 1) {
            foreach ($ttcmsFooter as $key => $text) {
