...
<div class="tt_pa_product-variants product-variants">
  {foreach from=$groups key=id_attribute_group item=group}
	...
	<div class="clearfix product-variants-item col-xs-6 col-sm-4{if $group.group_type == 'select'} product-variants-item-select{/if}">
	  <span class="control-label">{$group.name|escape:'htmlall':'UTF-8'}:</span>
	  {if $group.group_type == 'select'}
		<select
		  class="form-control form-control-select"
		  id="ot_group_{$id_attribute_group|intval}"
	  ...
	  {elseif $group.group_type == 'color'}
		<ul id="group_{$id_attribute_group|intval}" class="ttstickyAddToCart-group">
		  {foreach from=$group.attributes key=id_attribute item=group_attribute}
