...
include_once(_PS_MODULE_DIR_.'ttgooglemap/classes/ClassGooglemap.php');

class Ttgooglemap extends Module
{
    protected $config_form = false;

    public function __construct()
    {
        $this->name = 'ttgooglemap';
        ...
        $this->displayName = $this->l('TT - Google Map');
        $this->description = $this->l('Display google map on contact us page.');
    ...
    {
        return array(
            'module-ttgooglemap-googlemap' => array(
    ...
    {
        $settings = array(
            'TT_GOOGLE_KEY' => 'AIzaSyA-3t-iZiIPpsze7aXFrqDyPeZcTQG8erg',
            'TT_GOOGLE_LAT' => 26.00998700,
            'TT_GOOGLE_LONG' => -80.29447200,
            'TT_GOOGLE_TYPE' => 'roadmap',
            'TT_GOOGLE_ZOOM' => 8,
            'TT_GOOGLE_SCROLL' => 0,
            'TT_GOOGLE_TYPE_CONTROL' => 0,
            'TT_GOOGLE_STREET_VIEW' => 1,
            'TT_GOOGLE_ANIMATION' => 1,
            'TT_GOOGLE_POPUP' => 1
        );
        return $settings;
    }
    ...
    public function getContent()
    {
        ...
        if (((bool)Tools::isSubmit('submitTtgooglemapSettingModule')) == true) {
        ...
        if ((bool)Tools::isSubmit('statusttgooglemap')) {
            $output .= $this->updateStatusTab();
        }

        if ((bool)Tools::isSubmit('deletettgooglemap')) {
            $output .= $this->deleteGooglemap();
        }

        if (Tools::getIsset('updatettgooglemap') || Tools::getValue('updatettgooglemap')) {
            $output .= $this->renderGooglemapForm();
        } elseif ((bool)Tools::isSubmit('addttgooglemap')) {
            $output .= $this->renderGooglemapForm();
        } elseif (!$result) {
    ...
    protected function renderFormSettings()
    {
        ...
        $helper->submit_action = 'submitTtgooglemapSettingModule';
    ...
    protected function validateSettings()
    {
        ...
        if (Tools::isEmpty(Tools::getValue('TT_GOOGLE_KEY'))) {
            $errors[] = $this->l('Google key is required.');
        }

        if (Tools::isEmpty(Tools::getValue('TT_GOOGLE_LAT'))) {
            $errors[] = $this->l('Item is required.');
        } else {
            if (!Validate::isCoordinate(Tools::getValue('TT_GOOGLE_LAT'))) {
                $errors[] = $this->l('Bad latitude format');
            }
        }

        if (Tools::isEmpty(Tools::getValue('TT_GOOGLE_LONG'))) {
            $errors[] = $this->l('Item is required.');
        } else {
            if (!Validate::isCoordinate(Tools::getValue('TT_GOOGLE_LONG'))) {
                $errors[] = $this->l('Bad longitude format');
            }
        }

        if (Tools::isEmpty(Tools::getValue('TT_GOOGLE_ZOOM'))) {
            $errors[] = $this->l('Zoom is required.');
        } else {
            if (!Validate::isUnsignedInt(Tools::getValue('TT_GOOGLE_ZOOM'))) {
    ...
    protected function renderGooglemapForm()
    {
        ...
        if ((bool)Tools::getIsset('updatettgooglemap') && (int)Tools::getValue('id_tab') > 0) {
    ...
    public function hookDisplayBackOfficeHeader()
    {
        ...
        $this->context->controller->addJS($this->_path.'views/js/ttgooglemap_back.js');
    }
    ...
    public function hookHeader()
    {
        ...
        $status = true;
        $script = '//maps.google.com/maps/api/js?key='. Configuration::get('TT_GOOGLE_KEY') .'&amp;sensor=true';
        $load = '//maps.google.com/maps/api/js?key='. Configuration::get('TT_GOOGLE_KEY') .'&amp;sensor=true';

        if (!in_array($script, $this->context->controller->js_files) && !in_array($load, $this->context->controller->js_files)) {
            $status = false;
        }

        $defaultLat = Configuration::get('TT_GOOGLE_LAT');
        $defaultLong = Configuration::get('TT_GOOGLE_LONG');
        ...
        $this->context->controller->addCSS($this->_path.'/views/css/ttgooglemap_front.css');
        $this->context->controller->addJS($this->_path.'/views/js/ttgooglemap_front.js');

        return $this->display($this->_path, '/views/templates/hook/ttgooglemap-header.tpl');
    }

    public function hookDisplayHome()
    {
        return $this->display(__FILE__, 'views/templates/hook/ttgooglemap-front.tpl');
    }
