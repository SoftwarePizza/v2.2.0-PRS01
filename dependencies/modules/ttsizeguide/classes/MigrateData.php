...
class MigrateData
{
    ...
    {
        $this->loadedData = [];
        $texts = Db::getInstance()->executeS('SELECT i.`id_shop`, il.`id_lang`, il.`text` FROM `' . _DB_PREFIX_ . 'ttsizeguideinfo` i
            INNER JOIN `' . _DB_PREFIX_ . 'ttsizeguideinfo_lang` il ON il.`id_ttsizeguideinfo` = i.`id_ttsizeguideinfo`');
    ...
    public function insertData()
    {
        ...
        $ttSizeguides = array_intersect_key($this->loadedData, $shopsIds);

        $info = new ttsizeguideinfo();
        $info->text = reset($ttSizeguides);
        $return &= $info->add();

        if (count($ttSizeguides) > 1) {
            foreach ($ttSizeguides as $key => $text) {
