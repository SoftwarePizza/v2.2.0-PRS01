<?php
/**
 * 2015-2022 PrestaShop
 *
 * PrestaShop Product Custom Tab
 *
 * NOTICE OF LICENSE
 *
 ...
 *
 *  @author    PrestaShop
 *  @copyright 2015-2022 PrestaShop
...
include_once(_PS_MODULE_DIR_.'ttcustomtab/classes/ClassCustomTab.php');

class Ttcustomtab extends Module
{
    public function __construct()
    {
        $this->name = 'ttcustomtab';
        ...
        $this->displayName = $this->l('TT - Custom Tab');
    ...
    public function createAjaxController()
    {
        ...
        if (is_array($languages)) {
            foreach ($languages as $language) {
                $tab->name[$language['id_lang']] = 'ttcustomtab';
            }
        }
    ...
    public function getContent()
    {
        ...
        if ((bool)Tools::isSubmit('statusttcustomtab')) {
            $output .= $this->updateStatusTab();
        }

        if ((bool)Tools::isSubmit('deletettcustomtab')) {
            $output .= $this->deleteTab();
        }

        if (Tools::getIsset('updatettcustomtab') || Tools::getValue('updatettcustomtab')) {
            $output .= $this->renderTabForm();
        } elseif ((bool)Tools::isSubmit('addttcustomtab')) {
            $output .= $this->renderTabForm();
        } elseif (!$result) {
    ...
    {
        $this->_clearCache('ttcustomtab.tpl');
    }
    ...
    protected function renderTabForm()
    {
        ...
        if ((bool)Tools::getIsset('updatettcustomtab') && (int)Tools::getValue('id_tab') > 0) {
    ...
    public function hookDisplayBackOfficeHeader()
    {
        ...
        $this->context->controller->addJS($this->_path.'views/js/ttcustomtab_admin.js');
        $this->context->controller->addCSS($this->_path.'views/css/ttcustomtab_admin.css');
    }

    public function hookDisplayHeader()
    {
        $this->context->controller->addCSS($this->_path.'views/css/ttcustomtab.css');
    }
    ...
    public function hookDisplayProductTab()
    {
        ...
        if ($tabs) {
            ...
            $this->context->smarty->assign('ttcustomtabs', $result);
        }

        return $this->display(__FILE__, 'views/templates/hook/ttcustomtab.tpl');
    }
    ...
    public function hookDisplayProductTabContent()
    {
        ...
        return $this->display(__FILE__, 'views/templates/hook/ttcustomtab-content.tpl');
    }
}
