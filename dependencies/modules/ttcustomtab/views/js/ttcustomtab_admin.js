...
$(document).ready(function(){
    $('.bootstrap .ttcustomtab > tbody  tr  td.dragHandle').wrapInner('<div class="positions"/>');
    $('.bootstrap .ttcustomtab > tbody  tr  td.dragHandle').wrapInner('<div class="dragGroup"/>');
    initAjaxTabs();
    initAutocomplite();
});
...
function initAjaxTabs(){
    ...
    $('.ttcustomtab > tbody tr').each(function(){
        var id = $(this).find('td:first').text();
        $(this).attr('id', 'item_'+id.trim());
    });

    var $tabslides = $('.ttcustomtab > tbody');

    $tabslides.sortable({
        cursor: 'move',
        items: '> tr',
        update: function(event, ui){
            $('.ttcustomtab > tbody > tr').each(function(index){
...
function addProduct(event, data, formatted){
    ...
    $('#product_autocomplete_input').setOptions({
        extraParams: {excludeIds : getProductds()}
    });
