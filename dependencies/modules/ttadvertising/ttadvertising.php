...
class TtAdvertising extends Module implements WidgetInterface
{
    ...
    public function __construct()
    {
        ...
        $this->displayName = $this->l('TT - Advertising block');
    ...
    protected function initialize()
    {
        ...
        if ((Shop::getContext() == Shop::CONTEXT_GROUP || Shop::getContext() == Shop::CONTEXT_SHOP)
            && file_exists(_PS_MODULE_DIR_.$this->name.'/views/img/'.$this->adv_imgname.'-g'.$this->context->shop->getContextShopGroupID().'.'.Configuration::get('BLOCKADVERT_IMG_EXT'))
        ) {
            $this->adv_imgname .= '-g'.$this->context->shop->getContextShopGroupID();
        }
        if (Shop::getContext() == Shop::CONTEXT_SHOP
            && file_exists(_PS_MODULE_DIR_.$this->name.'/views/img/'.$this->adv_imgname.'-s'.$this->context->shop->getContextShopID().'.'.Configuration::get('BLOCKADVERT_IMG_EXT'))
        ) {
        ...
        if ($this->adv_imgname == 'advertising') {
            $this->adv_img = Tools::getMediaServer($this->name)._MODULE_DIR_.$this->name.'/views/img/fixtures/'.$this->adv_imgname.'.jpg';
        } else {
            $this->adv_img = Tools::getMediaServer($this->name)._MODULE_DIR_.$this->name.'/views/img/'.$this->adv_imgname.'.'.Configuration::get('BLOCKADVERT_IMG_EXT');
        }
    ...
    private function _deleteCurrentImg()
    {
        ...
        if ($this->adv_imgname != 'advertising' && file_exists(_PS_MODULE_DIR_.$this->name.'/views/img/'.$this->adv_imgname.'.'.Configuration::get('BLOCKADVERT_IMG_EXT'))) {
            unlink(_PS_MODULE_DIR_.$this->name.'/views/img/'.$this->adv_imgname.'.'.Configuration::get('BLOCKADVERT_IMG_EXT'));
        }
        ...
        if (Tools::isSubmit('submitAdvConf')) {
            if (isset($_FILES['adv_img']) && isset($_FILES['adv_img']['tmp_name']) && !empty($_FILES['adv_img']['tmp_name'])) {
                if ($error = ImageManager::validateUpload($_FILES['adv_img'], Tools::convertBytes(ini_get('upload_max_filesize')))) {
                    ...
                    if (!move_uploaded_file($_FILES['adv_img']['tmp_name'], _PS_MODULE_DIR_.$this->name.'/views/img/'.$this->adv_imgname.'.'.Configuration::get('BLOCKADVERT_IMG_EXT'))) {
        ...
        $fields_form = array(
            'form' => array(
                ...
                'input' => array(
                    ...
                    array(
                        'type' => 'switch',
                        'label' => $this->l('Display in the left column'),
                        'name' => 'left_column',
                        'is_bool' => true,
                        'values' => array(
                                array(
                    ...
                    array(
                        'type' => 'switch',
                        'label' => $this->l('Display in the right column'),
                        'name' => 'right_column',
                        'is_bool' => true,
                        'values' => array(
                            array(
