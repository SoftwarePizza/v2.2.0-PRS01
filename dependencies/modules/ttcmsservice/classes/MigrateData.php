...
class MigrateData
{
    ...
    {
        $this->loadedData = [];
        $texts = Db::getInstance()->executeS('SELECT i.`id_shop`, il.`id_lang`, il.`text` FROM `' . _DB_PREFIX_ . 'ttcmsserviceinfo` i
            INNER JOIN `' . _DB_PREFIX_ . 'ttcmsserviceinfo_lang` il ON il.`id_ttcmsserviceinfo` = i.`id_ttcmsserviceinfo`');
    ...
    public function insertData()
    {
        ...
        $ttcmsServices = array_intersect_key($this->loadedData, $shopsIds);

        $info = new ttcmsserviceinfo();
        $info->text = reset($ttcmsServices);
        $return &= $info->add();

        if (count($ttcmsServices) > 1) {
            foreach ($ttcmsServices as $key => $text) {
